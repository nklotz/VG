...
public:
  ...
  void doUserInput(Array<VRG3D::EventRef> &events)
  {
    ...
    for (int i=0;i<events.size();i++)
    {
      ...
      else if (events[i]->getName() == "kbd_CTRL_UP_down")
      {  _virtualToRoomSpace = CoordinateFrame(Vector3(0,0,0.1)) * _virtualToRoomSpace;
      }
      else if (events[i]->getName() == "kbd_CTRL_DOWN_down")
      {  _virtualToRoomSpace = CoordinateFrame(Vector3(0,0,-0.1)) * _virtualToRoomSpace;
      }
  ...
  void doGraphics(RenderDevice *rd)
  {
    ...
    //
      GLenum  E = glGetError();
      while(E != GL_NO_ERROR)
      {  std::cout<<"WHOOPS Flushing gl errors"<<std::endl;
          std::cout<<E<<std::endl;
          E = glGetError();
      }
